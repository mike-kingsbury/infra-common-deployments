...
Common dashboards (dora-metrics, generic-dashboards, perfscale, power-monitoring) are in `base/dashboards/`. Internal-only dashboards (kueue, kyverno, pipeline-service) are in `k-components/internal-dashboards/dashboards/` and are included by the `internal-*` overlays via a Kustomize Component.
